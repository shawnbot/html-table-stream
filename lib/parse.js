...
module.exports = function parse(options) {
  ...
  function onopentag(name, attrs) {
    ...
    if (!table) {
      ...
      if (match) {
        if (instance === ++count) {
          table = node;
        } else if (count < instance) {
          console.warn('skipping table @ %d (looking for %d)', count, instance);
        }
      }
    }
  }
  ...
  function processRow(tr) {
    if (!columns) {
      ...
      var cells = tr.querySelectorAll('td, th').filter(matchesContentCell);
      if (cells.length) {
        var row = readColumnsFromRow(tr, columns, matchesContentCell, log);
        log('read row:', row);
        rows.push(row);
      } else {
        log('skipping row (no matching cells):', tr.outerHTML);
      }
    } else {
